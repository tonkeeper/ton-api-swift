...
public typealias TonStreamingAPIV2APIChallengeHandler = ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
private var challengeHandlerStore = SynchronizedDictionary<Int, TonStreamingAPIV2APIChallengeHandler>()
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T>, @unchecked Sendable {
     ...
     */
    public var taskDidReceiveChallenge: TonStreamingAPIV2APIChallengeHandler?
    ...
    @discardableResult
    override open func execute(_ apiResponseQueue: DispatchQueue = TonStreamingAPIV2API.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
    ...
    open func buildHeaders() -> [String: String] {
        ...
        for (key, value) in TonStreamingAPIV2API.customHeaders {
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.TonStreamingAPIV2.\(UUID().uuidString)")
                }
